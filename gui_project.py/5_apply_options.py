...
def add_file():
    files =  filedialog.askopenfilenames(title='이미지 파일을 선택하세요', \
        ...
        initialdir=r'C:\Users\dnwnd\OneDrive\바탕 화면\PythonWorkspace') 
        # 최초의 경로를 보여줌
...
def merge_image():
    ...
    try:
        ...
        # 포맷
        img_format = cmb_format.get().lower() # 소문자로 변경 = lower()
        ...
        #     y_offset += img.size[1] # 높이값만큼 더해주기

        for idx, img in enumerate(images):
            # width가 원본유지가 아닐때 이미지 크기조정
            if img_width > -1:
                img = img.resize(image_sizes[idx])

            result_img.paste(img, (0,y_offset))
            y_offset += (img.size[1] + img_space) # 높이값 + 간격만큼 더해주기
