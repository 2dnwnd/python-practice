...
ball_images = [
    pygame.image.load(os.path.join(image_path, 'ballon.png')),
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 세로 벽 (스테이기에 튕겨서 올라가는 처리)
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ball_val['to_y'] = ball_val['init_speed_y']
        else: # 그 외에는 속도를 증가 (포물선 효과 처음 스피드가 마이너스라)
            ball_val['to_y'] += 0.5
    ...
    # 캐릭터 rect 정보 업데이트 (캐릭터가 계속 이동하므로)
    character_rect = character.get_rect()
    character_rect.left = character_x_pos
    character_rect.top = character_y_pos

    # 공 위치 정의 (위에 딕셔너리에서 받아오기)
    for ball_idx, ball_val in enumerate(balls):
        ball_pos_x = ball_val['pos_x']
        ball_pos_y = ball_val['pos_y']
        ball_img_idx = ball_val['img_idx']
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            # 충돌 체크 (ball_idx,weapon_idx는 0이상 )
            ...
            continue # 안쪽 for문의 조건이 맞지않으면 continue
        break 
    ...
    # 경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # 초로 바꾸기
